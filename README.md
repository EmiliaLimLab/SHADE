...
Alternatively, `polyROI_geojson2tsv.py` can be used to apply a manually defined bounding polygon rather than the one generated by `preprocess_he_otsu.py`. The user-specified polygon must be in the form of a .geojson object. The script also requires an argument specifying a tab-delimited samplesheet, where the first column contains absolute paths to the SVS images and the second column contains absolute paths to the geoJSON file with bounding polygon coordinates. This script can be run as follows:
